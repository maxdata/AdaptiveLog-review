...
class SentenceTransformer(nn.Sequential):
    ...
    def code_smart_batching_collate(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(tokens, label), ...]
        ...
        for item in texts[0]:
            for j in range(num_texts - 1):
                new_text[0].append(item)

        for i in range(len(texts[0])):
            for j in range(1,num_texts):
                new_text[1].append(texts[j][i])


        sentence_features = []
        for idx in range(2):
            tokenized = self.tokenize(new_text[idx])
            sentence_features.append(tokenized)
    ...
    def fit(self,
        ...
        if save_f:
            ...
            for dataloader in dataloaders:
                ...
                # dataloader.collate_fn = self.code_smart_batching_collate
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            # self.save(output_path + '_' + str(epoch))
        if save_task:
            # torch.save(loss_model,'/home/mlp/adaptive/model/discriminate_model.pt')
            torch.save(loss_model,small_model_path)
